# =============================================================================
#  SIMPLY-SUPPORTED SLAB OPTIMISATION
# -----------------------------------------------------------------------------
...
# ── OPTIMISATION ───────────────────────────────────────────────────────────
results_csv = f"results_{use_case}_{'_'.join(criteria)}.csv"
...
plot_datasets.print_comp_slab_table_from_csv(results_csv, lengths)
...
#     plt.title(
#         f"Best composite slab at L = {lengths[idx_vrc]:.1f} m\n"
...
# ── PLOT FORMATTING ───────────────────────────────────────────────────────
_title = (f"All slab types — simply-supported — {use_case.capitalize()} ({'/'.join(criteria)})  "
...
_fig_filenames = {
    1: os.path.join(_plot_dir, f"plot_{use_case}_1span_{'_'.join(criteria)}_height.png"),
    2: os.path.join(_plot_dir, f"plot_{use_case}_1span_{'_'.join(criteria)}_gwp.png"),
    3: os.path.join(_plot_dir, f"plot_{use_case}_1span_{'_'.join(criteria)}_mass.png"),
}
...
for fig_n, (info_left, info_right) in _fig_info.items():
    ...
    for sub_pos, info in enumerate([info_left, info_right], start=1):
        ...
        plt.grid()
    plt.suptitle(_title, fontsize=10)
