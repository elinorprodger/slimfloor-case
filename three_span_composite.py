# =============================================================================
#  THREE-SPAN CONTINUOUS SLAB OPTIMISATION
# -----------------------------------------------------------------------------
...
# ── ANALYSIS RANGE ─────────────────────────────────────────────────────────
n_spans  = 3                           # spans for RC / composite (timber stays at 1)
...
# ── OPTIMISATION ───────────────────────────────────────────────────────────
results_csv = f"results_{use_case}_3span_{'_'.join(criteria)}.csv"
...
if not REPLOT_ONLY:
    ...
    # ── RC SOLID (rectangular) — three-span continuous ────────────────────
    ...
    # ── COMPOSITE SLAB — three-span continuous ────────────────────────────
    mat_names = ["'ready_mixed_concrete'"]
...
# ── PLOT FORMATTING ───────────────────────────────────────────────────────
_title = (f"Three-span continuous — {use_case.capitalize()} ({'/'.join(criteria)})  "
          f"|  qk={qk:.0f} N/m², finishes={finishes_load:.0f} N/m²\n"
          f"RC solid and composite: three-span continuous  |  RC ribbed, timber solid, timber ribbed: simply-supported")
...
_fig_filenames = {
    1: os.path.join(_plot_dir, f"plot_{use_case}_3span_{'_'.join(criteria)}_height.png"),
    2: os.path.join(_plot_dir, f"plot_{use_case}_3span_{'_'.join(criteria)}_gwp.png"),
    3: os.path.join(_plot_dir, f"plot_{use_case}_3span_{'_'.join(criteria)}_mass.png"),
}
