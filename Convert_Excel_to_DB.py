...
excel_file = "210426_Datenbankdefinition.xlsx"   #name of excel file. must be in same directory as code
...
df_sheeting_prop = pd.read_excel(excel_file, sheet_name="sheeting_prop", engine="openpyxl")

# Verbindung zur SQLite-Datenbank herstellen (oder neu erstellen)
conn = sqlite3.connect("database_270426.db") #name of database
...
df_sheeting_prop.to_sql("sheeting_prop", conn, if_exists="replace", index=False)
